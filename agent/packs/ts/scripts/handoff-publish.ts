...
import { computeManifest, diffManifests, hasDrift, type Manifest } from "./checksum-gate.ts";
...
export function runHandoffPublish(cwd: string, producer: string): GateResult {
  ...
  const latest = readGuardLog(cwd).filter((e) => e.guard === "design-gate").at(-1);
  ...
  let stored: Manifest;
  try {
    stored = JSON.parse(readFileSync(join(cwd, ".bounded/contract-checksums.json"), "utf8")) as Manifest;
  ...
  for (const path of ["spec.md", ...Object.keys(current.files)]) {
  ...
  try {
    ...
    const selectors = [`${prefix}spec.md`, `${prefix}**/*.contract.ts`];
    const receipt = makeReceipt(cwd, producer, "design-gate", selectors);
    const expected = ["spec.md", ...Object.keys(current.files)].map((path) => `${prefix}${path}`).sort();
