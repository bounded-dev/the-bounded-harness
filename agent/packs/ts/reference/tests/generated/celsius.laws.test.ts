// GENERATED from celsius.contract.ts by packs/ts/scripts/value-object-laws.ts — do not edit.
...
describe("Celsius — value-object laws (generated)", () => {
  test("refuses every hostile input", () => {
    ...
    const wronglyAccepted = HOSTILE_INPUTS
      ...
      .filter(([, raw]) => {
        const result = Celsius.parse(raw);
  ...
  test("is equal by value, not by reference", () => {
    const a = mustParse(Celsius.parse(-273), "Celsius.parse(-273)");
    const b = mustParse(Celsius.parse(-273), "Celsius.parse(-273)");
  ...
  test("parses deterministically", () => {
    expect(mustParse(Celsius.parse(-273), "Celsius.parse(-273)")).toStrictEqual(mustParse(Celsius.parse(-273), "Celsius.parse(-273)"));
  });
  ...
  test("equals discriminates two different valid inputs", () => {
    const a = mustParse(Celsius.parse(-273), "Celsius.parse(-273)");
    const other = mustParse(Celsius.parse(20), "Celsius.parse(20)");
